...
async def interactive_loop(node: IoTNode):
    ...
    while True:
        ...
        if cmd == 'help':
            ...
            print("  send_inbox <text>             - Send Inbox message to Sink (E2E protected)")
            print("  stop_hb <downlink_nid>        - Stop forwarding heartbeats to a direct downlink")
            print("  start_hb <downlink_nid>       - Resume forwarding heartbeats")
            print("  blocked_hb                    - List blocked downlinks")
...
async def main(name: str = 'Device Node', adapter: Optional[str] = None):
    ...
    # Try to start a GATT server first so we can pass it to IoTNode
    gatt = None
    if BlueZGattServer is not None:
        try:
            ...
            def _on_gatt_write(data: bytes):
                # Will be called after node is created
                try:
                    import json
                ...
                try:
                    ...
                    print(f"[{name}] Error processing GATT write")
    ...
    node = IoTNode(name=name, is_sink=False, adapter=adapter, ble_gatt_server=gatt)
